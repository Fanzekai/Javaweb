...
if (a.equals("insert")) {
    ...
    Long tw = Query.make("nonghu").where("suozaitanwei", request.getParameter("suozaitanwei")).count();
    if ( tw>0){

        out.println("<script>alert('该摊位已分配，请勿重复分配！');location.replace(document.referrer);</script>");
        return  ;
    }


    String charuid = new CommDAO().insert(request, "nonghu", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    // 插入数据完成，弹出页面并跳转到指定页面
    ...
    String charuid = new CommDAO().update(request, "nonghu", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    Map map = Query.make("nonghu").find(Request.get("id"));

    new CommDAO().delete(request, "nonghu"); // 执行删除命令
