...
if (a.equals("insert")) {
    ...
    // 执行插入购物车模块,表名：gouwuche
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据
    ext.put("addtime", Info.getDateStr());

    String charuid = new CommDAO().insert(request, "gouwuche", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    new CommDAO().commOper("UPDATE gouwuche SET xiaoji=(shoujia*goumaishuliang) WHERE id='" + charuid + "'");
    ...
    // 执行更新购物车模块,表名：gouwuche
    HashMap ext = new HashMap(); // 创建Map对象,主要用于插入扩展数据，和覆盖前台提交的数据

    String charuid = new CommDAO().update(request, "gouwuche", ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法

    new CommDAO().commOper("UPDATE gouwuche SET xiaoji=(shoujia*goumaishuliang) WHERE id='" + request.getParameter("id") + "'");
    ...
    // 获取一行数据
    Map map = Query.make("gouwuche").find(Request.get("id"));

    new CommDAO().delete(request, "gouwuche"); // 执行删除命令
