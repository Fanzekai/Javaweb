...
public class CommDAO {
    ...
    // 数据库密码
    public static final String pwd = "Fzk";
    ...
    public List select(String sql, int pageno, int rowsize) {
        ...
        public static int sumXiaoliangByFenlei(String quyu){
            ...
            try
            {
                String sql = "SELECT SUM(xiaoliang) FROM shangpin WHERE quyu = ?";
                PreparedStatement statement = connection.prepareStatement(sql.toString());
                statement.setString(1, quyu);
    ...
    public static Map<Integer,String>sumXiaoliangByFenlei1(){

        Connection connection = getConn();
        Map<Integer,String > sumMap = new HashMap<>();
        try
        {
            String sql = "SELECT SUM(xiaoliang) as xiaoliang,q.quyuming as quyu FROM shangpin s join quyu q on s.quyu =q.id group by quyu";
            PreparedStatement statement = connection.prepareStatement(sql.toString());
            ResultSet resultSet = statement.executeQuery();
            while (resultSet.next()) {
                int xiaoliang = resultSet.getInt("xiaoliang");
                String quyu = resultSet.getString("quyu");

                sumMap.put(xiaoliang, quyu);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
